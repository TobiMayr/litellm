...
class TestToolChoiceForResponsesAPIResponse:
    ...
    the Chat Completion shape) crashed ResponsesAPIResponse's Pydantic validation with
    13 errors, since that field only accepts strings or the flat Responses API shape
    ({"type": "function", "name": "..."}). Reproduced on Bedrock/Anthropic models,
    which route through this litellm_completion bridge for the Responses API (OpenAI/
    Azure models use a native Responses API path and never hit this code).
    """
